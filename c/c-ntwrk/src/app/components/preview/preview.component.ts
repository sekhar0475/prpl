...
export class PreviewComponent implements OnInit {
  ...
  processChargeLeftArray = [];
  processChargeRightArray = [];
  ...
  renderPreviewData() {
    ...
    if(modeObj  !== undefined){
      this.previewList.routeAllocationPrev['routeMode'] = modeObj.descr;
    } else {
    ...
    if(paymentOptObj !== undefined) {
      this.previewList.paymentTermsPrev['lkpAssocNrmPayoutOptName'] = paymentOptObj.descr;
    } else {
    ...
    if(fuelTypeObj !== undefined){
      this.previewList.paymentTermsPrev['lkpFuelType'] = fuelTypeObj.descr;
    } else {
    ...
    if(fuelIndexObj !== undefined){
      this.previewList.paymentTermsPrev['lkpFuelIndex'] = fuelIndexObj.descr;
    } else {
    ...
    if(this.previewList.networkDeductionPrev.networkProcessCharges != undefined && this.previewList.networkDeductionPrev.networkProcessCharges.length > 0) { 
      ...
      let procChargeArrLength = this.previewList.networkDeductionPrev.networkProcessCharges.length; 

      if(procChargeArrLength > 0) {
        this.processChargeLeftArray = this.previewList.networkDeductionPrev.networkProcessCharges.slice(0,(Math.ceil(procChargeArrLength/2)));
        this.processChargeRightArray = this.previewList.networkDeductionPrev.networkProcessCharges.slice((Math.ceil(procChargeArrLength/2)),procChargeArrLength);
      }
    }
    this.spinner.hide();
