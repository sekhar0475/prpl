...
export class CompareVersionComponent implements OnInit {
 ...
 // end header footer
      ...
      // file save and send
        ...
        let dataOb = {associateEmailId:ob.toEmail, 
          assocId:AppSetting.associateId,
          cntrctCode : this.obj1.cntrCode ? this.obj1.cntrCode : "NOT GENERATED YET",
          ver : this.obj1.ver ? this.obj1.ver : "1"
        }
  ...
  getOperationalDays(daysArr) {
    ...
    if (daysArr !== undefined) {
      daysArr.forEach((element, index) => {
        let obj = this.days.find(x => x.id == element);
        if (obj !== undefined) {
          if (index !== 0) {
            operationalDays = operationalDays + ', ' + obj.name;
          } else {
            operationalDays = obj.name;
          }
        }
      });
    }
  ...
  renderPreviewData(obj: any) {
    ...
    if (modeObj !== undefined) {
      obj.routeAllocationPrev['routeMode'] = modeObj.descr;
    } else {
    ...
    if (paymentOptObj !== undefined) {
      obj.paymentTermsPrev['lkpAssocNrmPayoutOptName'] = paymentOptObj.descr;
    } else {
    ...
    if (fuelTypeObj !== undefined) {
      obj.paymentTermsPrev['lkpFuelType'] = fuelTypeObj.descr;
    } else {
    ...
    if (fuelIndexObj !== undefined) {
      obj.paymentTermsPrev['lkpFuelIndex'] = fuelIndexObj.descr;
    } else {
    ...
    obj.processChargeLeftArray = [];
    obj.processChargeRightArray = [];
    if(obj.networkDeductionPrev.networkProcessCharges != undefined && obj.networkDeductionPrev.networkProcessCharges.length > 0) { 
      ...
      let procChargeArrLength = obj.networkDeductionPrev.networkProcessCharges.length; 
        
      if(procChargeArrLength > 0) {
        obj.processChargeLeftArray = obj.networkDeductionPrev.networkProcessCharges.slice(0,(Math.ceil(procChargeArrLength/2)));
        obj.processChargeRightArray = obj.networkDeductionPrev.networkProcessCharges.slice((Math.ceil(procChargeArrLength/2)),procChargeArrLength);
      }
