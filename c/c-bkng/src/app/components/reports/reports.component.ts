...
export class ReportsComponent implements OnInit {
  ...
  typeChange() {
    ...
    if (this.reportName === 'ASSOCIATE_CONTRACT_ASSOCIATE_LEVEL_REPORT') {
      this.isDisabled = true;
      this.searchByVal = "ALL";
      // this.finalObj.searchBy = null;
    } else {
  ...
  submit() {
    ...
    this.apiService.getReportDownload(body).subscribe((res: any) => {
      ...
      let filename = `booking_${this.reportName
        }__report_${new Date().getTime()}.${this.reportFormat
        }`.toLowerCase();
      this.spinner.hide();
